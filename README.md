...
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/liquid-staking-core`](https://github.com/clarity-fintech/liquid-staking-core)
...
**Liquid Staking Core** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/liquid-staking-core
...
| `bin/misc mis/kernel/LiquidStakingCoreKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/LiquidStakingCoreKernel.mis --check --compact-letters
...
```
mis/kernel/LiquidStakingCoreKernel.mis
mis/packs/mclrty_mint.mis
mis/packs/non_custodial.mis
mis/packs/redeem_invariant.mis
mis/packs/reward_accrue.mis
mis/packs/yield_token.mis
...
mis/commands/LiquidStakingCoreCommandCatalog.mis
mis/commands/LiquidStakingCoreCommands.mis
mis/commands/LiquidStakingCoreNetworkBind.mis
...
`mis/kernel/LiquidStakingCoreKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: Liquid Staking Core Kernel

module LiquidStakingCoreKernel {
  ...
  invariant domain_bound: domain == liquid_staking;
  ...
  invariant pack_mclrty_mint_flag: pack_mclrty_mint_bound == true;
  invariant pack_yield_token_flag: pack_yield_token_bound == true;
  invariant pack_non_custodial_flag: pack_non_custodial_bound == true;
  invariant pack_reward_accrue_flag: pack_reward_accrue_bound == true;
  invariant pack_redeem_invariant_flag: pack_redeem_invariant_bound == true;
...
// Creator: Chandler William Ferguson
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Liquid Staking Core · MClrtyMint

module LiquidStakingCoreMClrtyMint {
  invariant letter_hash_bound: letter_hash_root != @0;
  invariant deep_root_moniversive: deep_root == moniversive;
  invariant settlement_chain: chain_id == 1202;
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_mclrty_mint_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant mclrty_mint_001: mclrty_mint_001_bound == true;
  invariant mclrty_mint_002: mclrty_mint_002_bound == true;
  invariant mclrty_mint_003: mclrty_mint_003_bound == true;
  invariant mclrty_mint_004: mclrty_mint_004_bound == true;
  invariant mclrty_mint_005: mclrty_mint_005_bound == true;
  invariant mclrty_mint_006: mclrty_mint_006_bound == true;
  invariant mclrty_mint_007: mclrty_mint_007_bound == true;
  invariant mclrty_mint_008: mclrty_mint_008_bound == true;
```
...
```
CONNECTED clrty-1/1202 liquid-staking-core
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/LiquidStakingCoreCommands.mis
```
...
{
  "ok": true,
  "module": "LiquidStakingCoreBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1161,
...
*Reproduce:* `make check` → `"ok": true`, module `LiquidStakingCoreBacklinkIndex`, 9 invariants, 4 outcomes, 1161 typed letters.
...
- [clarity-fintech/liquid-staking-core](https://github.com/clarity-fintech/liquid-staking-core) — this repository
